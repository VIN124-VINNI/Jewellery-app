...
//                   <button onClick={() => toggleMenu(item._id)} className="text-xl">⋮</button>
//                   {openMenuId === item._id && (
//                     <div className="absolute right-0 mt-2 w-32 bg-white border shadow rounded z-30 flex flex-col">
//                       <button
//                         onClick={() => navigate(`/edit/${item._id}`, { state: item })}
//                         className="bg-orange-500 text-white px-4 py-2 rounded hover:bg-purple-600"
//                       >
...
//                 {/* ✅ Clickable Card */}
//                 <div onClick={() => navigate(`/Jewellery/${item._id}`)} className="cursor-pointer">
//                   <Image
//                     path={item?.image}
//                     alt={item?.name}
//                     styling="w-full h-60 object-cover rounded mb-2 hover:scale-105 transition-transform duration-300"
//                   />
//                   <h2 className="text-xl font-semibold text-center">{item?.name}</h2>
//                   <p className="text-sm text-gray-600 text-center">{item?.category}</p>
//                   <p className="mt-1 text-gray-800 text-center">Cost: ₹{item?.cost}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
          {filteredJewellery.length > 0 ? (
            filteredJewellery.map(item => (
              <div
                key={item?._id}
                className="relative border-2 border-gray-200 p-3 rounded-lg shadow hover:shadow-lg transition-all duration-300 bg-white"
              >
                {/* Menu button */}
                <div className="absolute top-2 right-2 z-20">
                  <button onClick={() => toggleMenu(item._id)} className="text-xl">⋮</button>
                ...
                <div onClick={() => navigate(`/Jewellery/${item._id}`)} className="cursor-pointer">
                  <Image
                    path={item?.image}
                    alt={item?.name}
                    styling="w-full h-60 object-cover rounded mb-3 hover:scale-105 transition-transform duration-300"
                  />
                  <h2 className="text-lg md:text-xl font-semibold text-center">{item?.name}</h2>
                  <p className="text-sm text-gray-600 text-center">{item?.category}</p>
                  <p className="mt-1 text-gray-800 text-center">Cost: ₹{item?.cost}</p>
                  <p className="text-sm text-gray-500 text-center">Editor: {item?.editor}</p>
                </div>
